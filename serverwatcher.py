...
handler = logging.FileHandler(os.getcwd() + '/mylog.log')
...
def is_disabled_time():
    ...
    # systemctlコマンドの結果パターン：'水 2022-09-21 15:26:25 UTC'の曜日を削り、経過時間を取得
    res = subprocess.run('systemctl show --property=ActiveEnterTimestamp minecraft.service | cut -d= -f2',
                         shell=True, capture_output=True, text=True)
    started = dt.strptime(res.stdout[2:-1], '%Y-%m-%d %H:%M:%S %Z')
